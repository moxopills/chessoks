...
(function() {
    ...
    const roomUI = window.LobbyRoomUI;
    ...
    async function loadWaitingRoom() {
        ...
        try {
            ...
            roomUI?.renderWaitingRoomCard({
                card: waitingRoomCard,
                infoEl: waitingRoomInfo,
                enterEl: waitingRoomEnter,
                room,
                onEnter: () => {
    ...
    async function loadActiveGame() {
        ...
        try {
            ...
            if (signature === lastActiveGameSignature) {
                return;
            }
            lastActiveGameSignature = signature;
            roomUI?.renderActiveGameCard({
                card: activeGameCard,
                infoEl: activeGameInfo,
                enterEl: activeGameEnter,
                room,
                onEnter: () => {
                    window.location.href = `/games/${room.id}/`;
                },
    ...
    async function loadBoardPreview() {
        ...
        try {
            const data = await API.get('/community/boards/posts/', { limit: 5, no_count: 1 });
    ...
    function renderBoardPreview(posts) {
        roomUI?.renderBoardPreview({
            container: lobbyBoardList,
            posts,
    ...
    function buildRoomSignature(room) {
        return roomUI?.buildRoomSignature(room) || '';
    }
